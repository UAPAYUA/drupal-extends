...
class UaPayApi
{
  ...
  function __construct($config = null)
  {
    ...
    $this->testMode = ($config['TEST_MODE'] == '1') ? 1 : 0;
    $this->type_payment = ($config['TYPE_PAYMENT'] == '1') ? 1 : 0;
    $this->user_redirect_url = !empty($config['REDIRECT_URL']) ? trim($config['REDIRECT_URL']) : '';
...
//	}
  ...
  public function setDataTypeOperation($type = '')
  {
    ...
    if (!empty($type)) {
      $this->payload['data']['type'] = strval(self::OPERATION_HOLD);
    } else {
  ...
  public function getRedirectUrl()
  {
    if (!empty($this->user_redirect_url))
      return htmlspecialchars_decode($this->user_redirect_url);
    else
      return $this->payload['data']['redirectUrl'];
  }
...
//	}
  ...
  {
    return !empty($this->type_payment) ? self::OPERATION_HOLD : self::OPERATION_PAY;
  }
...
//		self::writeLog(['http_info' => $http_info, 'errNum' => $errNum]);

    if ($errMsg) {
      self::writeLog($errMsg, 'Error Message');
    }
//		self::writeLog($server_response, 'response');
...
//        $this->messageError = $this->formalizeErrorMessage($result['error']);
        ...
        $this->messageErrorCode = $result['error']['code'];
      }

      if (!empty($result['data']['token'])) {
//        self::writeLog($this->parseSign($result['data']['token']), 'parse token');
